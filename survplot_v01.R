...
survplot <- function(fit, cause = 1, byval = 12, tstart = 0, xlab = '', order = 0, stacked = F, title = '', colormap = 'Set1', bw = F, legendpos = 'topright', lgd = '') {
  
  if (length(cause) == 2 & stacked == F) par(mfrow = c(1, 2))
  ...
  for (k in 1:length(cause)) {
    ...
    if ((ngroups == 1 & length(cause) == 1) | bw == T) { 
      cols <- rep('#000000', ngroups + 1)
    ...
    if (fit$type == 'right') {
      ...
      # base plotting
      plot(fit[, .cause], conf.int = FALSE, bty = 'l', axes = FALSE, col = cols, ylim = c(0, 1), main = title[k], lty = linetype)
      ...
      if (order[1] != '') {
        ...
        plot(y = inc.cs[, 1], x = fit$time, bty = 'l', axes = FALSE, ylim = c(0, 1), 
             type = 's', col = cols[1], ylab = '', xlab = '', lty = linetype)
  ...
  if (bw & ngroups > 1)  {
    legend(legendpos, lty = linetype, legend = varnames, bty = 'n')
    lgd = ''
  }
  if (length(lgd) > 1) legend(legendpos, lty = linetype, legend = lgd, bty = 'n')
