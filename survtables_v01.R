...
med <- function(fit, digits = 1, cause = 1, timescale = 'months', caption = '', internal = F) {
     ...
     r1 = cbind(r1, r2)
     df <- data.frame(r1, medfu = medfu.str, row.names = NULL)
...
tableone <- function(data, by = '', margin = 2, cat.names = '', cont.names = '', caption = '', digits = 1, test = F, df = F) {
     ...
     df.cnt  <- data.frame(data[, !isfactor])
     
     for (i in 1:ncol(df.cat)) df.cat[, i] <- droplevels(df.cat[, i])
     
     lvls <- lapply(df.cat, levels)
               ...
               if (test) {
                    cat.p <- sapply(df.cat, function(x) {
                      if (length(levels(x)) == 1) {
                        ...
                        return(suppressWarnings(round(chisq.test(x, y = byvar)$p.value, 3))) 
                      }
                    })
     ...
     if (ncol(df.cnt) > 0) {
          ...
          if (test) {
               if (nlevels(byvar) == 2) {
                    cnt.p <- data.frame(p = sapply(df.cnt, function(x) { 
                      suppressWarnings(round(wilcox.test(x ~ byvar)$p.value, 3))}))
               } else {
                    cnt.p <- data.frame(p = sapply(df.cnt, function(x) { 
                      suppressWarnings(round(kruskal.test(sd$age_TX1 ~ byvar)$p.value, 3))}))
               }
...
mvatab <- function(fit, caption = '', digits = 2, df = F) {
     ...
     if (!relsurvobj) {
          tmp.tab2 <- cbind(round(tmp.tab$conf.int[, c(1, 3, 4)], digits), 
...
survtab <- function(fit, times, cause = 1, digits = 0, caption = '', outcomes = '', median = F, test = T) {
     ...
     suppressWarnings(column_names[column_names == ''] <- outcomes)
